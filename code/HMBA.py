...
MAC25CYNO_TEMPLATE_CREATION_TIME = datetime.datetime(2026, 4, 4, tzinfo=datetime.timezone.utc)
...
MAC25CYNO_HOMBA_ANNOTATION_CREATION_TIME = datetime.datetime(2025, 4, 4, tzinfo=datetime.timezone.utc)
...
MAC25_TEMPLATE_SUMMARY = "The macaque Mac25Rhesus template is an averaged MRI template from 25 M. mulatta subjects. Full documentation and provenance can be found in the HMBA 2025 data release (https://brain-map.org/consortia/hmba/hmba-release-basal-ganglia)."
RIKEN25_TEMPLATE_SUMMARY = "The marmoset RIKEN25 template is an averaged MRI template from 25 C. jacchus subjects. Full documentation and provenance can be found in the HMBA 2025 data release (https://brain-map.org/consortia/hmba/hmba-release-basal-ganglia)."
...
MAC25CYNO_TEMPLATE_SUMMARY = "The macaque Mac25Cyno template is an averaged MRI template from 25 M. cynomologous subjects and resampled to 160 um isotropic resolution."
...
MAC25CYNO_HOMBA_ANNOTATION_DESCRIPTION = "The 2025 and initial release of M. cynomolgous parcellations of the basal ganglia and associated structures described in the Harmonized Ontology of Mammalian Brain Anatomy (HOMBA). Details and process of parcellation are described in Ding et al. 2025. Labeling was performed on a 2D histological series. Labels were registered to the animal's pre-mortem MRI template volume and subsequently to the Mac25Cyno template resampled to ."
...
def create_all_anatomical_templates(input_dir: Path, results_dir: Path, library, scales):
    ...
    template = Template(name="hmba-adult-rhesusmacaque-mri-template", version="2025", scales=scales["mac25"])
    template.create(average_template_prefix, results_dir)
    library.add(template)
    logging.info(f"Created average_template: {template.name} {template.version}")
    ...
    _write_template_data_description(
        ...
        project_name="hmba-adult-rhesusmacaque-mri-template",
        modalities=[Modality.MRI],
        creation_time=MAC25_TEMPLATE_CREATION_TIME,
    )

    # Create macaque mac25 cynomolgous template
    average_template_prefix = input_dir / "average_template" / "mac25cyno_template" / "20260404" / "macaque_mac25cyno_template"
    template = Template(name="hmba-adult-cynomolgousmacaque-mri-template", version="2026", scales=scales["mac25"])
    template.create(average_template_prefix, results_dir)
    library.add(template)
    logging.info(f"Created average_template: {template.name} {template.version}")

    # Write data description for the mac25 cynomolgous template
    _write_template_data_description(
        output_dir=template.location(results_dir),
        name=template.name,
        version=template.version,
        summary=MAC25CYNO_TEMPLATE_SUMMARY,
        project_name="hmba-adult-cynomolgousmacaque-mri-template",
        modalities=[Modality.MRI],
        creation_time=MAC25CYNO_TEMPLATE_CREATION_TIME,
    )
...
def create_all_ccf_annotation_sets(input_dir: Path, results_dir: Path, library, scales):
    ...
    template_mac25 = library.get_template("hmba-adult-rhesusmacaque-mri-template", "2025")
    template_mac25cyno = library.get_template("hmba-adult-cynomolgousmacaque-mri-template", "2026")
...
# Define annotation configurations for different CCF versions
    annotations = [
        ...
        },
        {
            ...
            "version": "2025",
            "name": "hmba-adult-rhesusmacaque-homba-annotation",
            "scale": scales["mac25"],
        ...
        },
        {
            ...
            "template": template_mac25cyno,
            "version": "2026",
            "name": "hmba-adult-cynomolgousmacaque-homba-annotation",
            "scale": scales["mac25"],
...
def package_ccf(input_dir, output_dir, library, scales):
    ...
    anatomical_space = CoordinateSpace(
        name="hmba-adult-cynomolgousmacaque-mri-space",
        version="2026",
        template=library.get_template("hmba-adult-cynomolgousmacaque-mri-template", "2026"),
    )
    library.add(anatomical_space)
    ...
    atlases = [
        ...
        Atlas(
            name="hmba-adult-cynomolgousmacaque-homba-atlas",
            version="2025",
            coordinate_space=library.get_coordinate_space(name="hmba-adult-cynomolgousmacaque-mri-space", version="2026"),
            annotation_set=library.get_annotation_set(
                name="hmba-adult-cynomolgousmacaque-homba-annotation", version="2026"
            ),
            terminology=library.get_terminology(
                name="hmba-mammalian-homba-terminology", version="2025"
            ),
